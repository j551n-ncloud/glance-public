...
Planned, not built yet. Tracked as GitHub issues, contributions welcome.

- **Garmin 2FA support** ([#1](https://github.com/j551n-ncloud/glance-public/issues/1)):
  the current unofficial login breaks on any account with MFA enabled. Bootstrap
  OAuth tokens once via a flow that handles the 2FA prompt (Python's `garth`
  does), then load them into `GARMIN_TOKENS_FILE` so the proxy never logs in
  with a password at all.
- **Official Strava login flow** ([#2](https://github.com/j551n-ncloud/glance-public/issues/2)):
  replace the manual browser plus `curl` token bootstrap with a proper OAuth
  callback served by `ics-proxy`, so connecting Strava is a "Connect" click that
  stores the refresh token itself instead of a hand-edited `.env`.
- **intervals.icu support** ([#3](https://github.com/j551n-ncloud/glance-public/issues/3)):
  read wellness, fitness/fatigue/form (CTL, ATL, TSB) and planned workouts from
  the intervals.icu API, as a data source alongside Strava and Apple Health, and
  feed them into `coach_briefing`.

## License

Apache License 2.0, see [`LICENSE`](LICENSE) and [`NOTICE`](NOTICE). You may
use, modify and redistribute this project, including commercially, but
attribution is a condition, not a courtesy:

- Keep every copyright, patent and attribution notice you received (§4c).
- Ship the [`NOTICE`](NOTICE) file with any distribution, and reproduce its
  contents in your own documentation or in a display generated by the work
  (§4d). That is the line crediting this repository as the original.
- Mark any file you changed as changed (§4b), so a fork cannot present this
  code as its own from scratch.

The license covers the code in this repository only. [Glance](https://github.com/glanceapp/glance)
itself and every npm/Docker dependency remain under their own licenses, listed
in [`NOTICE`](NOTICE).
